...
public class Stock {
    ...
    public void modifierStock (Panier panier){
        ...
        for (int i = 0; i < panier.getItems().size(); i++){
           ...
           // il parcourt le panier récupère le item panier qui contient la référance et la quantité, il cherche ensuite dans le tableau le prodduit qui a la même référence
           // il récupére la quantité de de l'item , il prend la quantité déja présente dans le produit correspondant il déduit d'elle la quantité du panier 
        }
